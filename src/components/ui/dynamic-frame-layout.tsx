      ...
      <div className="relative w-full h-full overflow-hidden">
        ...
        >
          ...
          >
            {failed ? (
              fallback ? (
                <img src={fallback} alt="Rasa es krim" className="w-full h-full object-cover" loading="lazy" />
              ) : (
                <div className="w-full h-full flex items-center justify-center bg-[var(--muted)] text-2xl" aria-hidden>🍦</div>
              )
            ) : (
              <video
    ...
    >
      {frames.map((frame) => {
        ...
        return (
          <motion.div
            key={frame.id}
            className="relative focus-within:ring-2 focus-within:ring-[var(--ring)] rounded-xl"
            tabIndex={0}
            role="button"
            aria-label={`Rasa ${frame.id}`}
            style={{
              transformOrigin,
          ...
          >
            <FrameComponent
              video={frame.video}
              fallback={(frame as any).fallback}
              width="100%"
              height="100%"
